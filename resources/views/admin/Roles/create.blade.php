    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form method="POST" class='row' action="{{ route('admin.roles.store') }}" enctype="multipart/form-data">
                    @csrf
                                ...
                                <div class="tab">
                                    <ul class="nav nav-tabs" role="tablist">
                                        @foreach($models as $model)
                                            <li class="nav-item">
                                                <a class="nav-link text-blue" data-toggle="tab" href="#{{ $model }}" role="tab" aria-selected="false">{{$model}}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                        ...
                                        @foreach ($models as $index=>$model)
                                            <div class="tab-pane {{ $index == 0 ? 'active' : '' }}" id="{{ $model }}" role="tabpanel">
                                                <div class="pd-20">
                                                    ...
                                                    <label><input type="checkbox" id="cb{{ $model }}" class="check-all" >check_all</label> <br>

                                                    @foreach ($maps as $map)
                                                        <label><input type="checkbox" class="cb{{ $model }}" name="permissions[]" value="{{ $map . '-' . $model }}">{{$map}}</label> <br>
                                                    @endforeach
                                                </div>
